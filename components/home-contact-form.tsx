...
export function HomeContactForm() {
  ...
  const errorLines: Line[] = useMemo(() => {
    ...
    for (const k of present) {
      const padded = `> checking ${k}`.padEnd(32, '.');
  ...
  const validate = (): Errors => {
    ...
    else if (form.message.trim().length < 10) e.message = 'message must be ≥ 10 chars';
    return e;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTxCtx({
        email: form.email,
        bytes: new Blob([form.message]).size,
      });
      setPhase('sending');
    } catch (err) {
  ...
  return (
    <div className="relative">
      ...
      <div
        key={shakeKey}
        className={`border border-border ${shakeKey > 0 && isErrPhase ? 'shake-on-error' : ''}`}
      >
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center px-4 bg-black/70 backdrop-blur-sm popup-enter">
      ...
      >
        <div className="flex items-center gap-2 px-4 py-3 border-b border-primary/40 bg-primary/10">
          ...
          <span className="text-xs font-mono text-primary ml-3">contact.sh</span>
          {isSent && (
            <button
              type="button"
              onClick={onClose}
        ...
        <div className="p-6 font-mono text-sm space-y-1.5 min-h-[280px] scanline">
