...
export default function EditPostPage() {
  ...
  if (notFound) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center font-mono text-muted-foreground">
        Post not found. <Link href="/admin/dashboard" className="text-primary ml-2 hover:underline">← Back</Link>
      </div>
    );
  }
  ...
  const set = (key: keyof Blog, value: unknown) =>
      ...
      <header className="border-b border-border px-6 py-4 bg-sidebar sticky top-0 z-10">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3 text-sm font-mono">
            ...
            <span className="text-foreground truncate max-w-[200px]">{form.title || 'Edit Post'}</span>
          </div>
          <div className="flex items-center gap-2">
            {form.published ? (
              <button
                ...
                className="px-4 py-2 text-xs font-mono border border-border text-muted-foreground hover:border-amber-400/50 hover:text-amber-400 transition-all disabled:opacity-50"
              >
                Unpublish
              </button>
            ) : (
              <button
                ...
                className="px-4 py-2 text-xs font-mono border border-primary/30 text-primary hover:bg-primary/5 transition-all disabled:opacity-50"
              >
                Publish
              </button>
            )}
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        <div className="flex items-center gap-4 mb-8">
          <h1 className="text-2xl font-bold font-mono glow-text">
            <span className="text-primary">//</span> Edit Post
          </h1>
          <span className={`text-xs px-2.5 py-1 border font-mono ${form.published ? 'text-primary border-primary/40 bg-primary/5' : 'text-muted-foreground border-border'}`}>
